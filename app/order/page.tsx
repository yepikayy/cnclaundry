...
const services = [
  { id: "wash-fold", name: "Cuci & Lipat", price: "Rp 10K/kg", min: "Min. 2 kg" },
  { id: "dry-cleaning", name: "Dry Cleaning", price: "Mulai Rp 35K/item", min: "Per item" },
  { id: "ironing", name: "Setrika & Press", price: "Rp 8K/item", min: "Per item" },
  { id: "sneakers", name: "Cuci Sepatu", price: "Mulai Rp 80K/pasang", min: "Per pasang" },
];
...
function StepIndicator({ step, current }: { step: number; current: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <motion.div
        animate={{
          backgroundColor: done ? "#0ea5e9" : active ? "#0ea5e9" : "#f1f5f9",
          scale: active ? 1.1 : 1,
        }}
        ...
        >
          ...
          <div className="bg-sky-50 rounded-2xl p-4 text-left mb-8 space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="text-slate-700 font-medium">{services.find((s) => s.id === form.service)?.name}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-slate-500">Tanggal</span>
              <span className="text-slate-700 font-medium">{form.date}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-slate-500">Waktu</span>
              <span className="text-slate-700 font-medium">{form.time}</span>
            </div>
            <div className="flex justify-between text-sm">
          ...
          <button
            onClick={() => { setSubmitted(false); setStep(0); setForm({ service: "", date: "", time: "", name: "", phone: "", address: "", notes: "" }); }}
                  ...
                  {services.map((s) => (
                    <motion.button key={s.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                      ...
                      className={`p-5 rounded-2xl border-2 text-left transition-all ${
                        form.service === s.id ? "border-sky-400 bg-sky-50 shadow-md shadow-sky-100" : "border-slate-100 hover:border-sky-200"
                      }`}
                    >
                      <div className="font-semibold text-slate-800 mb-1">{s.name}</div>
                      <div className="text-sky-500 text-sm font-medium">{s.price}</div>
                      <div className="text-slate-400 text-xs mt-1">{s.min}</div>
                      {form.service === s.id && (
                      ...
                      {timeSlots.map((t) => (
                        <button key={t} onClick={() => set("time", t)}
                          className={`py-2.5 px-3 rounded-xl border text-sm font-medium transition-all ${
                            form.time === t ? "border-sky-400 bg-sky-50 text-sky-600" : "border-slate-100 text-slate-600 hover:border-sky-200"
                          }`}
                        >
                          {t}
                        </button>
              ...
              <motion.div key="step2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.3 }}>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    { label: "Nomor Telepon", field: "phone", type: "tel", placeholder: "+62 812-3456-7890" },
                    { label: "Alamat Penjemputan", field: "address", type: "text", placeholder: "Jl. Merdeka No. 5, Jakarta" },
                  ...
                  <div>
                    ...
                    <textarea rows={3} placeholder="Contoh: detergen bebas pewangi, taruh di depan pintu..."
            ...
            {step === 3 && (
              <motion.div key="step3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.3 }}>
                ...
                <div className="bg-sky-50 rounded-2xl p-6 space-y-4 mb-6">
                  {[
                    ["Layanan", services.find((s) => s.id === form.service)?.name ?? ""],
                    ["Tanggal", form.date],
            ...
            {step < 3 ? (
              <motion.button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-xl font-semibold text-sm transition-all ${
                  canNext() ? "bg-sky-500 text-white hover:bg-sky-600 shadow-md shadow-sky-100" : "bg-slate-100 text-slate-400 cursor-not-allowed"
                }`}
              >
              ...
              <motion.button
                ...
                className="flex items-center gap-2 px-8 py-3 rounded-xl bg-sky-500 text-white font-bold text-sm shadow-md shadow-sky-100 hover:bg-sky-600 transition-all"
              >
