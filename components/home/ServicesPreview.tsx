...
const DEFAULT_ICON = (
  <svg viewBox="0 0 48 48" className="w-10 h-10" fill="none" xmlns="http://www.w3.org/2000/svg">
    <circle cx="24" cy="24" r="20" fill="rgba(186,230,253,0.4)" />
    <circle cx="24" cy="24" r="8" fill="white" stroke="rgba(14,165,233,0.6)" strokeWidth="1.5" />
  </svg>
);

export function ServicesPreview({ data }: { data: Service[] }) {
  return (
          ...
          {data.map((s, i) => (
            <ScrollReveal key={s.id} delay={i * 0.1}>
              <Link href={`/services#${s.slug}`}>
                ...
                >
                  <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold mb-4 ${s.badge_color}`}>
                    {s.badge}
                  </span>
                  <div className="mb-4">{ICONS[s.slug] ?? DEFAULT_ICON}</div>
                  <h3 className="font-bold text-slate-800 text-lg mb-2">{s.name}</h3>
                  <p className="text-slate-500 text-sm leading-relaxed">{s.subtitle}</p>
